    ...
    with torch.no_grad():
        with model.ema_scope():
            for batch in tqdm(data, desc="Data"):
                ...
                shape = [1, 6, opt.H//4, opt.W//4, opt.D//4]
                ...
                recon_src = model.decode_first_stage(z_src).detach().cpu()
                
                save_nifti(x_samples_ddim, os.path.join(sample_path, os.path.join( f"{subject_id}_{opt.source}_to_{opt.target}.nii.gz")))
                save_nifti(recon_src, os.path.join(sample_path, os.path.join( f"{subject_id}_{opt.source}_recon.nii.gz")))
