...
from ddpm.data.base import Txt2ImgIterableBaseDataset
...
class DataModuleFromConfig(pl.LightningDataModule):
    def __init__(self, batch_size, train=None, validation=None, test=None, predict=None,
        ...
        if validation is not None:
            ...
            self.val_dataloader = self._val_dataloader #partial(self._val_dataloader, shuffle=shuffle_val_dataloader)
        if test is not None:
            self.dataset_configs["test"] = test
            self.test_dataloader = self._test_dataloader # partial(self._test_dataloader, shuffle=shuffle_test_loader)
...
def calculate_max_memory_usage(model, input_size, c_size):
    # Create a dummy input tensor with the given input size
    dummy_input = torch.zeros(*input_size, dtype=torch.float32)
    dummy_input_c = torch.zeros(*c_size, dtype=torch.float32)
    
    # Move model to GPU if available
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)
    dummy_input = dummy_input.to(device)
    dummy_input_c = dummy_input_c.to(device)
    
    # Calculate memory for model parameters
    element_size = torch.tensor(0, dtype=torch.float32).element_size()
    param_memory = count_parameters(model) * element_size
    
    # Perform a forward pass with the dummy input
    activation_sizes = []
    def hook_fn(module, input, output):
        if isinstance(output, (tuple, list)):
            activation_sizes.append(sum(tensor_size(o) for o in output))
        else:
            activation_sizes.append(tensor_size(output))
    ...
    model(dummy_input, dummy_input_c)
    ...
    # Calculate memory for optimizer states (assuming Adam optimizer)
    optimizer_memory = param_memory * 2  # for first and second moments in Adam

    # Calculate total maximum memory usage
