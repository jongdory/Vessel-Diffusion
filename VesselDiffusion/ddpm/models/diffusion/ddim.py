...
from ddpm.modules.diffusionmodules.util import make_ddim_sampling_parameters, make_ddim_timesteps, noise_like
...
class DDIMSampler(object):
    ...
    def ddim_sampling(self, cond, shape,
                      ...
                      callback=None, timesteps=None, skip_timesteps=None, quantize_denoised=False,
        ...
        if skip_timesteps is not None:
            img = self.model.q_sample(x0, torch.full((b,), skip_timesteps, device=device, dtype=torch.long))
        else:
        ...
        for i, step in enumerate(iterator):
            if skip_timesteps is not None and step > skip_timesteps:
                continue
